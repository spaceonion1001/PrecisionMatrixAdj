...
from sklearn.metrics import DistanceMetric, pairwise_distances_chunked, f1_score, precision_score, recall_score, precision_recall_curve, average_precision_score, accuracy_score, pairwise_distances, silhouette_score
...
def run_aad(args, features, labels, gmms):
    ...
    return gmms, num_anomalies_queried, num_anomalies_queried_list
...
def main_true(args):
    ...
    #         init_precisions = np.array([np.ones(features.shape[1]) for _ in range(trial_k)])
    #         gmm = GaussianMixture(
    #             n_components=trial_k,
    ...
    # evaluate_kmeans(features, k_range=range(2, 11), save_path="./results_kmeans", dataset_name=args.data)
    # exit()
    ...
    for seed in range(42, 46):
        ...
        if full_covariance:
            ...
            gmm = GaussianMixture(n_components=k, covariance_type='full', random_state=seed, precisions_init=init_precisions, n_init=5)
            ...
            gmm = GaussianMixture(n_components=k, covariance_type='diag', random_state=seed, precisions_init=init_precisions, n_init=5)
        ...
        # print("GMM Precisions {}".format(gmm.precisions_))
        # print("GMM Covariance Shape {}".format(gmm.covariances_.shape))
        gmms = []
        gmms_init = []
        ...
        gmms, num_anomalies_queried, num_anomalies_queried_list = run_aad(args, features, labels, gmms)
        ...
        with open('./results_gmms/precision_scores_{}_{}.txt'.format(args.data, args.save_suffix), 'a') as f:
